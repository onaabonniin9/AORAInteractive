...
namespace Microjuego1  // ← exactamente igual
{
    public class UIManager : MonoBehaviour
    {
        ...
        public void MostrarFichasRonda(int fichas)
        {
            StartCoroutine(AnimarFichasRonda(fichas));
        }

        private IEnumerator AnimarFichasRonda(int fichas)
        {
            phaseText.text = $"MEMORIZA {fichas} MONEDAS";
            phaseText.color = new Color(0f, 1f, 0.8f);
            StartCoroutine(PunchText(phaseText.transform));
